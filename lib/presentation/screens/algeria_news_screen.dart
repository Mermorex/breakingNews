...
class SourceExtractor {
  static final Map<String, String> _domainMappings = {
    'tsa-algerie': 'TSA',
    'lesoirdalgerie': 'Le Soir d\'Algérie',
    'echoroukonline': 'Echorouk',
    'ennaharonline': 'Ennahar',
    'elkhabar': 'El Khabar',
...
class _AlgeriaNewsScreenState extends State<AlgeriaNewsScreen>
  ...
  // Ideally use NewsSources.algerian if available, otherwise keep local definition
  List<NewsSource> get _rssSources => [
        ...
        NewsSource(
            ...
            url: 'https://www.echoroukonline.com/rss/'),
        NewsSource(
            name: 'Ennahar Online', url: 'https://www.ennaharonline.com/rss/'),
      ];
  ...
  Future<void> _generateDailyRecap() async {
    ...
    try {
      ...
      final recap = await MistralService().generateDailyRecap(
        ...
        isArabic: isArabic,
        topic: isArabic ? "الجزائر" : "Algeria",
      );
              ...
              children: [
                      ...
                      Text(
                        isDisabled
                            ? (_isArabicContent
                                ...
                                : 'Loading news feed...')
                            : (_isArabicContent
                                ? 'ملخص ذكي للأخبار الجزائرية'
                                : 'AI summary of Algerian events'),
                        style: AppFonts.body(
                ...
                if (_isDailyRecapLoading)
                  ...
                  Icon(Icons.arrow_forward_ios_rounded,
                      color:
                          isDisabled ? Colors.white24 : AppColors.accentPurple,
                      size: 18),
              ],
            ),
          ),
        ...
        child: Column(
          ...
          children: [
            SizedBox(
                ...
                height: 40,
                child: CircularProgressIndicator(
                    color: AppColors.accentOrange, strokeWidth: 3)),
  ...
  Widget _buildFeaturedCard(RssItemModel article) {
    return _AlgerianMainArticleCard(
      article: article,
  ...
  Widget _buildCompactCard(RssItemModel article) {
    return _AlgerianSideArticleCard(
      article: article,
...
class _AlgerianMainArticleCard extends StatefulWidget {
  ...
  const _AlgerianMainArticleCard({
  ...
  @override
  State<_AlgerianMainArticleCard> createState() =>
      _AlgerianMainArticleCardState();
}

class _AlgerianMainArticleCardState extends State<_AlgerianMainArticleCard>
...
class _AlgerianSideArticleCard extends StatefulWidget {
  final RssItemModel article;
  final Color accentColor;
  final bool isArabic;
  final bool isSummaryArabic;
  ...
  const _AlgerianSideArticleCard({
    required this.article,
    required this.accentColor,
    required this.isArabic,
    required this.isSummaryArabic,
  ...
  @override
  State<_AlgerianSideArticleCard> createState() =>
      _AlgerianSideArticleCardState();
}

class _AlgerianSideArticleCardState extends State<_AlgerianSideArticleCard>
    with SingleTickerProviderStateMixin {
  bool _isExpanded = false;
  bool _isLoading = false;
  String? _summary;
